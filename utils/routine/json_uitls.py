...
class BaseJsonOperation:
    ...
    def save_json(self, data: dict, out_path: str):  # 如果out_path 中不存在给定的路径,会报错哦,例如 ./result/t.json,因为不存在result 文件夹会报错
        with open(out_path, 'w', encoding="utf-8") as fw:
            logger.info(f'{out_path}保存成功')
            json.dump(data, fw, indent=4, ensure_ascii=False)

    def load_json(self, in_path):
        # logger.critical(f'正在解析{os.path.basename(in_path)}')
...
class JsonOperation():
    ...
    def fetch_json_path(self):
        ...
        json_suffix = '.json'
        self.json_path = self.file_path.replace(fp_suffix, json_suffix).replace(' ',' ').replace('jusda/', '')
        # 检查下神抽存在self.json_path
        # json_path_basename=os.path.exists()
        # os.path.exists()
