...
from cache_online.tree_set import TreeSet
...
class AbstractDoubleArrayTrie:
    ...
    def add2trie(self, word):
        ...
        words_codes = [self.alphabet_dict[x] for x in word
                       if x is not None and x in self.alphabet_dict.keys()]
    ...
    def find(self, chars):
        try:
            if chars is None or chars == "":
                return False
            word_set = self.fuzzy_search(chars)
            if word_set is not None and len(word_set) > 0:
                if chars in word_set:
                    return True
            return False
        except Exception as e:
            logging.info("查询出错{}".format(chars))
        return False
                            ...
                            if len(chars) > 1:
                                if chars[1] in self.alphabet_dict:
                                    ...
                                    if self.if_legal(state=transition, transition=tmp_next_grandson):
                                        ...
                                        # 交换操作
                                        tmp_queue.append((chars[1] + chars[0] + chars[2:], path, tol - 1, transition,
                                                          tmp_next_grandson))
